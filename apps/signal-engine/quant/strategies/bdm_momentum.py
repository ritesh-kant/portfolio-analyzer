"""Strategy G — BDM-Momentum (BDM-M) on Nifty Midcap 150.

Pre-registered hypothesis: research/hypotheses/2026-05-23-bdm-momentum.md
Rules-based v1 — no ML model.  DSR n_trials starts at 1 (clean experiment).

Strategy F (BDM) post-mortem: mean return 277.8 bps was strong but win rate
49.1% and Sharpe 0.173 failed the gate.  Root cause: bulk deals in stocks
below their medium-term trend are "bottom fishing" positions where continuation
probability is lower.  Strategy G adds a single filter: the stock must be
above its 50-day EMA at the close of signal date T.
...
Trading rule (hypothesis §5):
  ...
  2. Momentum filter: stock close on signal day T must be > 50-day EMA of
     close computed over the 50 trading days ending at T.
  3. Entry: T+1 open (next session after bulk deal disclosure at ~16:00)
  4. Exit: T+20 close (20 trading sessions after entry)
  5. Skip events within ±30 calendar days of a Lok Sabha general election.
  6. Skip events for stocks flagged by the promoter pledge filter (Strategy C).
  7. Aggregate multiple bulk deals for the same (symbol, date): one event.
...
Gate criteria (hypothesis §4 — immutable):
  ...
  - Total dev events (after momentum filter)   >= 30

Run the gate check:
  python -m quant.research.run --strategy g --split dev
"""
...
# ── Momentum parameters (pre-registered, immutable) ───────────────────────────
_EMA_SPAN = 50            # 50-day EMA of close price
_EMA_MIN_PERIODS = 25     # require at least 25 days of history to compute EMA
...
class TradeRecord:
    ...
    deal_value_cr: float
    client_name: str
    ema50: float           # 50-day EMA at signal date T
    close_t: float         # close price at signal date T
    slippage_draw: float = 0.0


def compute_ema50(ohlcv: pd.DataFrame) -> pd.Series:
    """Compute the 50-day EMA of close price for every (date, symbol) pair.

    Parameters
    ----------
    ohlcv : pd.DataFrame
        PIT-correct OHLCV indexed by (business_date, symbol), column 'close'.
        Must include at least 50 trading days before the earliest signal date
        to warm up the EMA properly.

    Returns
    -------
    pd.Series indexed by (business_date, symbol), values = EMA50 of close.
    """
    if ohlcv.empty or "close" not in ohlcv.columns:
        return pd.Series(dtype=float)

    # Unstack to (date × symbol) matrix
    close = ohlcv["close"].unstack(level="symbol")

    # EMA with adjust=False: standard recursive formula, min_periods enforced
    ema = close.ewm(span=_EMA_SPAN, adjust=False, min_periods=_EMA_MIN_PERIODS).mean()

    # Stack back to (date, symbol) Series
    return ema.stack()
...
def apply_momentum_filter(
    ...
    """Filter events to those where close_T > EMA50_T.
    ...
    ohlcv : pd.DataFrame
        PIT-correct OHLCV indexed by (business_date, symbol).
    ema50 : pd.Series or None
        Pre-computed EMA50 series (avoids recomputing when called multiple times).
        If None, compute_ema50(ohlcv) is called internally.

    Returns
    -------
    (filtered_events, n_momentum_skipped)
    """
    if events.empty:
        return events, 0
    ...
    for _, ev in events.iterrows():
        ...
        try:
            ...
            # No OHLCV or EMA for this event — skip (conservative)
            momentum_skipped += 1
            logger.debug("No OHLCV/EMA for %s on %s — skipped", sym, event_date)
            continue

        if np.isnan(ema_t):
            # Insufficient history to compute EMA — skip
            momentum_skipped += 1
            logger.debug("EMA NaN for %s on %s (insufficient history)", sym, event_date)
            continue
    ...
    filtered = pd.DataFrame(filtered_rows).reset_index(drop=True) if filtered_rows else \
               pd.DataFrame(columns=events.columns)
    return filtered, momentum_skipped
...
def simulate_trades(
    ...
    """Simulate BDM-Momentum trades from a pre-built events DataFrame.

    Parameters
    ----------
    events : pd.DataFrame
        From build_events() — one row per (symbol, event_date).
        Should already be filtered by universe; momentum filter is applied here.
    ohlcv : pd.DataFrame
        PIT-correct OHLCV indexed by (business_date, symbol).
        Must span at least 50 trading days before the earliest event.
    slippage_scale : float
        1.0 = nominal; 2.0 = cost-stress.
    rng : np.random.Generator or None
        For stochastic slippage in cost-stress simulation.
    ema50 : pd.Series or None
        Pre-computed EMA50 (pass to avoid recomputation on repeat calls).
    ...
    for _, ev in events.iterrows():
        ...
        # ── Momentum filter: close_T > EMA50_T ────────────────────────────────
...
def compute_gate_metrics(trades: list[TradeRecord], n_trials: int = 1) -> dict:
    ...
    Note: the event-count gate for Strategy G is ≥ 30 (reduced from F's ≥ 40
    because the momentum filter reduces the available event count).
    """
    ...
    if len(trades) < 30:          # ← 30 for G (30, not 40); pre-registered
        fail_reasons.append(f"n_trades {len(trades)} < 30")
...
def run_anti_strategy(
    ...
    """Backtest the inverse signal: SHORT same momentum-filtered events."""
