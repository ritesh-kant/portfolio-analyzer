...
scrapes RSS/NSE/BSE directly and on demand, so it works even while the
...
async def _fetch_all() -> list[dict[str, Any]]:
    """Gather RSS + NSE + BSE concurrently. A single feed failing (network,
    layout change) never blanks out the other two."""
    rss_result, nse_result, bse_result = await asyncio.gather(
        fetch_all_rss(), fetch_nse_announcements(), fetch_bse_announcements(),
        return_exceptions=True,
    )
    articles: list[dict[str, Any]] = []
    for label, result in (("rss", rss_result), ("nse", nse_result), ("bse", bse_result)):
