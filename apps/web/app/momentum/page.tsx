...
function TradeRow({ trade, active, onClick }: { trade: MomentumTrade; active: boolean; onClick: () => void }) {
  ...
  return <button onClick={onClick} className={`w-full border-b border-black/5 px-3 py-3 text-left transition last:border-0 hover:bg-accent/5 ${active ? 'bg-accent/10' : ''}`}>
    <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className="font-display font-semibold">{trade.symbol}</span><span className="rounded-full bg-black/5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-ink/60">{trade.setup.replaceAll('_', ' ')}</span></div><p className="mt-1 text-xs text-ink/55">Buy {money(trade.entry_price)} · {at(trade.entry_time)}{trade.exit_price !== undefined ? ` → Sell ${money(trade.exit_price)} · ${at(trade.exit_time)}` : ' · Open'}</p></div><div className={`text-right text-sm font-bold ${pnlClass(net)}`}>{net === undefined ? 'Open' : `${net >= 0 ? '+' : ''}${money(net)}`}<p className="mt-1 text-[11px] font-medium text-ink/45">{trade.exit_reason?.replaceAll('_', ' ') ?? 'in progress'}</p></div></div>
  </button>;
}
...
function EntryReason({ trade }: { trade: MomentumTrade }) {
  ...
  const gates = attention
    ? [
        ...
        { label: '1-minute confirmation', detail: `A green candle closed in its upper 40% with at least 2.5× recent one-minute volume${volumeRatio !== undefined ? `; recorded confirmation volume was ${volumeRatio.toFixed(2)}×.` : '.'}` },
