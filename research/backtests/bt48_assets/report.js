...
function ensureSeries(c) {
  ...
  /* BT48: on the 1-minute view the MACD panel is the ENGINE's series (warmed
     with prior sessions), not the browser's recomputation from today alone. */
  if (tf === "1m" && c.tr.eng) data.forEach(function (b, i) {
    var e = c.tr.eng[i]; b.macd = e[0]; b.signal = e[1]; b.hist = e[2];
  });
  c.series = {
    tf: tf, all: data,
    decFull: barIndex(data, c.tr.decision_time),
...
function drawChart(c) {
  ...
  /* BT48: the pause the micro pullback needed, and the candle the MACD check ran on */
  ...
  if (dIdx >= 0 && dIdx < data.length) {
    ...
    el("text", { x: x(dIdx) + 5, y: PRICE_TOP + PRICE_H - 8, fill: "#fbbf24", "font-size": 11 }, svg,
       "MACD check " + tr.decision_time + (tf === "5m" ? " (1m candle)" : ""));
  }
  ...
  if (tf === "1m" && dIdx >= 1 && dIdx < data.length) {
    ...
    el("text", { x: Math.min(x(dIdx) + 8, W - R - 250), y: MACD_TOP + 24, fill: "#fde68a",
                 "font-size": 11 }, svg,
       "hist " + tr.hist_prev.toFixed(3) + " → " + tr.hist_now.toFixed(3)
       + "  (−" + tr.dip_pct.toFixed(1) + "%; strict needs a rise, buffer allows −"
       + tr.tolerance_pct + "%)");
  }
  line(svg, data.map(function (b) { return b.macd; }), x, MY, "#fbbf24", 1.4);
  line(svg, data.map(function (b) { return b.signal; }), x, MY, "#a78bfa", 1.4);
  el("text", { x: L + 4, y: MACD_TOP + 10, fill: "#a9bac9", "font-size": 11 }, svg, "MACD 12/26/9" + (tf === "1m" ? " · engine, warmed" : " · browser, unwarmed"));
...
function buildCards() {
  ...
  DATA.trades.forEach(function (tr, i) {
    ...
    meta.innerHTML =
      ...
      + "<br>MACD check <b>" + tr.decision_time + "</b>: histogram <b>" + tr.hist_prev.toFixed(3)
      + " → " + tr.hist_now.toFixed(3) + "</b> (−" + tr.dip_pct.toFixed(1) + "%)"
      + " · pause " + (tr.pause.length ? tr.pause.join(", ") : "–")
      + " · engine re-check: strict <b>" + nice(tr.strict_reason.replace("attention_", ""))
      + "</b>, buffer <b>" + nice(tr.buffer_reason) + "</b>" + (tr.rule_check_ok ? " ✓" : " ⚠")
      + "<br>after entry: best <b>" + (tr.mfe_r >= 0 ? "+" : "") + tr.mfe_r.toFixed(2) + "R</b>"
      + ", worst <b>" + tr.mae_r.toFixed(2) + "R</b> (lows incl. the exit candle; a stop fills at −1R)"
      + " · strict rule on " + tr.symbol + " this day: "
      + (tr.strict_same_day.length ? tr.strict_same_day.map(function (s) {
          return "traded " + s.in + "→" + s.out + " " + nice(s.exit) + " " + pct(s.gross_pct);
        }).join("; ") : "<b>no trade</b>");
...
function init() {
  ...
  [["MACD check candle", "#fbbf24"], ["pause candle(s) (grey band)", "#94a3b8"],
