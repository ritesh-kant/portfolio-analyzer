...
function ensureSeries(c) {
  ...
  c.series = {
    ...
    // BT45: the bar the OTHER arm exited on, resolved the same way as the
    // real exit so the ghost marker lands on the same time axis.
    altFull: c.tr.alt_exit_time ? barIndex(data, c.tr.alt_exit_time) : -1
  };
  resetView(c);
  return c.series;
}
...
function drawChart(c) {
  ...
  // BT45: the cost-aware stop and the other arm's exit must both be visible,
  // otherwise the whole point of the comparison can fall outside the frame.
  if (tr.cost_stop) pv.push(tr.cost_stop);
  if (tr.alt_exit) pv.push(tr.alt_exit);
  ...
  /* candlestick formations on THIS timeframe only. Labels are staggered onto
     rows: two formations whose brackets sit close together on the x axis
     would otherwise print their names on top of each other. */
  var patternRowEnd = [];
  tr.patterns
    .filter(function (m) { return m.timeframe === tf; })
    .map(function (m) {
      var s0 = barIndexExact(all, m.start), s1 = barIndexExact(all, m.end);
      return { m: m, s0: s0, s1: s1 };
    })
    .filter(function (p) { return p.s0 >= 0 && p.s1 >= 0 && p.s1 >= viewStart && p.s0 < viewStart + data.length; })
    .sort(function (p, q) { return p.s0 - q.s0; })
    .forEach(function (p) {
      var m = p.m;
      var a = Math.max(0, p.s0 - viewStart), b2 = Math.min(data.length - 1, p.s1 - viewStart);
      var x0 = x(a) - cw, x1 = x(b2) + cw;
      var label = nice(m.name) + " · " + m.timeframe + " · " + m.strength.toFixed(2) + "×";
      var xc = (x0 + x1) / 2, halfW = label.length * 2.9 + 4;
      var row = 0;
      while (row < patternRowEnd.length && xc - halfW < patternRowEnd[row]) row++;
      patternRowEnd[row] = xc + halfW;
      ...
      el("text", { x: xc, y: PRICE_TOP + 31 + row * 11, "text-anchor": "middle",
                   fill: "#fde68a", "font-size": 10 }, g, label);
    });
  ...
  /* BT45 — the cost-aware stop: where the stop is lifted to once the trade is
     up 1R, so that a sale there covers the real round-trip charges instead of
     leaving the trade down by them. Drawn before the other lines so the entry
     and stop labels sit on top of it. */
  if (tr.cost_stop && tr.cost_stop >= vMin && tr.cost_stop <= vMax) {
    var csY = Y(tr.cost_stop);
    el("line", { x1: L, x2: W - R, y1: csY, y2: csY, stroke: "#fbbf24",
                 "stroke-width": 1.6, "stroke-opacity": ".95",
                 "stroke-dasharray": "7 3 2 3" }, svg);
    el("text", { x: W - R - 2, y: csY - 4, "text-anchor": "end", fill: "#fde68a",
                 "font-size": 11, "font-weight": "600" }, svg,
       "Cost stop " + money(tr.cost_stop) + "  (+" + tr.cost_stop_lift_pct.toFixed(2) + "% "
       + "= fees covered)");
  }

  /* trade levels */
  [["Stop", tr.stop, "#fb7185"], ["2R reference (no target order)", tr.target, "#34d399"]].forEach(function (r) {
  ...
  /* BT45 — the counterfactual: where this same trade exited WITHOUT the
     cost-aware stop. Hollow marker and a fainter line, so it reads as "what
     would have happened" rather than as a second real trade. */
  if (tr.alt_exit && tr.alt_exit >= vMin && tr.alt_exit <= vMax) {
    var aY = Y(tr.alt_exit);
    el("line", { x1: L, x2: W - R, y1: aY, y2: aY, stroke: "#94a3b8",
                 "stroke-opacity": ".8", "stroke-dasharray": "1 4" }, svg);
    var aLbl = "no rule: " + money(tr.alt_exit) + " " + tr.alt_exit_reason
             + " @" + tr.alt_exit_time;
    el("text", { x: L + 4, y: Math.abs(aY - exitY) >= 13 ? aY + 12 : exitY + 25,
                 fill: "#cbd5e1", "font-size": 11 }, svg, aLbl);
    var aIdx = S.altFull >= 0 ? S.altFull - viewStart : -1;
    if (aIdx >= 0 && aIdx < data.length)
      el("path", { d: "M " + (x(aIdx) - 6) + " " + (aY - 13) + " L " + (x(aIdx) + 6) + " "
                   + (aY - 13) + " L " + x(aIdx) + " " + (aY - 3) + " Z",
                   fill: "none", stroke: "#cbd5e1", "stroke-width": 1.4 }, svg);
  }
...
function buildCards() {
  ...
  DATA.trades.forEach(function (tr, i) {
    ...
    // BT45 — the whole point of this report: the two arms side by side.
    var cmp = h("div", { class: "cmp" }, sec);
    cmp.innerHTML =
      "<div>with the cost stop: <b>" + money(tr.exit) + "</b> " + nice(tr.exit_reason)
      + " @" + tr.exit_time + "</div>"
      + "<div>without it: <b>" + money(tr.alt_exit) + "</b> " + nice(tr.alt_exit_reason)
      + " @" + tr.alt_exit_time + "</div>"
      + "<div>cost stop sat at <b>" + money(tr.cost_stop) + "</b> = entry +"
      + tr.cost_stop_lift_pct.toFixed(2) + "% (the charges)</div>"
      + "<div>the rule was worth <b class='" + (tr.delta_inr >= 0 ? "g" : "r") + "'>"
      + inr(tr.delta_inr) + "</b> here — <b>" + tr.verdict + "</b></div>";
...
function render() {
  ...
  keep.slice().sort(function (a, b) {
    ...
    // BT45: this column is the rule's VALUE on that trade (cost arm minus the
    // no-rule arm at real charges), not the trade's own net.
    h("td", { class: "num " + cls(c.tr.delta_inr) }, tr, inr(c.tr.delta_inr));
    tr.onclick = function () {
...
function init() {
  ...
  [["up candle", "#2dd4bf"], ["down candle", "#fb7185"], ["EMA9 / MACD", "#fbbf24"],
   ...
   ["2R reference — the deployed arm sells on trend rules, never at a target", "#34d399"], ["▱ formation", "#fde68a"],
   ["cost stop (fees covered)", "#fbbf24"],
   ["exit WITHOUT the rule ▽", "#cbd5e1"]].forEach(function (p) {
    var s = h("span", {}, leg);
