...
| [news-event-type-drift](2026-06-16-news-event-type-drift.md) | news_trader_event_type | registered | 2026-06-16 | — | The signal-side experiment. Needs NEW classifier `event_type` field + ~4–6 forward weeks (n≥30 info-events) — CANNOT use existing 79 trades. Hypothesis: value-changing events (M&A/earnings/order-win/regulatory/capital-action) drift ≥+0.30%/trade while generic-PR ≈0; current classifier dilutes the two → BT9's gross≈0. KILL if Group-A gross <+0.15% OR (A−B) spread <+0.15% OR label-permuted anti ≥ real. **KILL = committed stop: declare news-trader non-viable, no fifth variation.** |
